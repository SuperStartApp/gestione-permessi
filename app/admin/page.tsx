...
import { UserPlus, Copy, Link as LinkIcon, ArrowLeft, LayoutDashboard, Trash2, Pencil, Check, X } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const[editingId, setEditingId] = useState<string | null>(null); // Per la modifica
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <button onClick={() => router.push('/admin/home')} className="p-2 hover:bg-gray-200 rounded-full transition text-red-900">
              <ArrowLeft size={24} />
            </button>
            <h1 className="text-3xl font-bold text-gray-800">Gestione Dipendenti</h1>
          </div>
          <button onClick={() => router.push('/admin/dashboard')} className="flex items-center gap-2 bg-red-900 text-white px-4 py-2 rounded-lg hover:bg-red-800">
            <LayoutDashboard size={20} /> Torre di Controllo
          </button>
        </header>
